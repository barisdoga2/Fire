...
#include "EasyUtility.hpp"
	...
	bool EasyPlayground::Render(double _dt)
	{
		...
		// Render
		{
			...
			if (imgui_showNormals)
			{
				normalDebugShader.Start();

				normalDebugShader.LoadUniform("view", camera.view_);
				normalDebugShader.LoadUniform("proj", camera.projection_);
				...
				for (EasyModel* model : objs)
				{
					for (const auto& kv : model->instances)
					{
						...
						{
							normalDebugShader.LoadUniform("model",
				...
				for (Chunk* chunk : chunks)
				{
					...
					normalDebugShader.LoadUniform("model", glm::translate(glm::mat4x4(1), glm::vec3(chunk->coord.x * Chunk::CHUNK_SIZE, 0, chunk->coord.y * Chunk::CHUNK_SIZE)));
	...
	void EasyPlayground::ImGUIRender()
	{
		...
		ImGui::Checkbox("Show Normals", &imgui_showNormals);
		ImGui::Checkbox("Show Normal Lines", &imgui_showNormalLines);
	...
	{
		normalDebugShader = EasyShader("NormalDebug");
		normalDebugShader.Start();
		normalDebugShader.BindAttribs({ "aPos", "aUV", "aNormal" });
		normalDebugShader.BindUniforms({ "model", "view", "proj" });
		normalDebugShader.Stop();
		...
		shader.BindUniforms({ "diffuse", "view", "proj", "model", "animated", "uCameraPos" });
	...
	void EasyPlayground::key_callback(GLFWwindow * window, int key, int scancode, int action, int mods)
	{
		...
		{
			ChunkRenderer::tri = !ChunkRenderer::tri;
			return;
		}
