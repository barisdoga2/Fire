...
public:
	...
	bool running;
	std::thread run;
	std::vector<EasySchedEntity*> tasks;
	steady_clock::time_point start;

	EasySched() : running(false)
	{
	...
	void AddTask(std::string name, milliseconds period, milliseconds offset, std::function<void(std::atomic<bool>&)> task)
	{
		...
		EasySchedEntity* entity = new EasySchedEntity();;
		entity->id = ++id_ctr;
		entity->name = name;
	...
	void Start()
	{
		...
		running = true;
		run = std::thread(&EasySched::Run, this);
	}
