...
EasyShader* HDR::equirectangularToCubemapShader = nullptr;
EasyShader* HDR::irradianceShader = nullptr;
EasyShader* HDR::prefilterShader = nullptr;
EasyShader* HDR::brdfShader = nullptr;

const unsigned int HDR::size = 512u;
const unsigned int HDR::irradianceSize = HDR::size / 16u;
const unsigned int HDR::prefilterSize = HDR::size / 4u;
const unsigned int HDR::sampleCount = 1024u;
...
HDR::HDR(std::string name)
{
	...
	float* data = stbi_loadf(std::string(GetRelPath("res/images/") + name + ".hdr").c_str(), &width, &height, &nrComponents, 0);
	...
	// ----------------------------------------------------------------------
	glUseProgram(equirectangularToCubemapShader->prog);
	equirectangularToCubemapShader->LoadUniform("equirectangularMap", 0);
	equirectangularToCubemapShader->LoadUniform("projectionMatrix", captureProjection);
	...
	for (unsigned int i = 0; i < 6; ++i)
	{
		equirectangularToCubemapShader->LoadUniform("viewMatrix", captureViews[i]);
	...
	// -----------------------------------------------------------------------------
	glUseProgram(irradianceShader->prog);
	irradianceShader->LoadUniform("environmentMap", 0);
	irradianceShader->LoadUniform("projectionMatrix", captureProjection);
	irradianceShader->LoadUniform("sampleDelta", sampleDelta);
	...
	for (unsigned int i = 0; i < 6; ++i)
	{
		irradianceShader->LoadUniform("viewMatrix", captureViews[i]);
	...
	// ----------------------------------------------------------------------------------------------------
	glUseProgram(prefilterShader->prog);
	prefilterShader->LoadUniform("environmentMap", 0);
	prefilterShader->LoadUniform("projectionMatrix", captureProjection);
	prefilterShader->LoadUniform("resolution", (float)HDR::size);
	prefilterShader->LoadUniform("sampleCount", HDR::sampleCount);
	...
	for (unsigned int mip = 0; mip < maxMipLevels; ++mip)
	{
		...
		prefilterShader->LoadUniform("roughness", roughness);
		for (unsigned int i = 0; i < 6; ++i)
		{
			prefilterShader->LoadUniform("viewMatrix", captureViews[i]);
	...
	glUseProgram(brdfShader->prog);
	brdfShader->LoadUniform("sampleCount", sampleCount);
	glClear(GL_COLOR_BUFFER_BIT | GL_DEPTH_BUFFER_BIT);
	RenderQuad();
