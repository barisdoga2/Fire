...
{
	player = new MainPlayer(network, Model(MAIN_CHARACTER), uid, glm::vec3(0, 0, 0));
	...
	for (const sPlayerInfo& info : infos)
	{
		...
		if (it == players.end())
		{
			...
			else
			{
				...
				players.push_back(new Player(network, Model(MAIN_CHARACTER), info.uid, false, glm::vec3(0, 0, 0)));
			}
		}
	}
...
void EasyPlayground::ProcessReceived(double _dt)
{
